...
(define (filtra-par-lg lst)
  (filter even? (filter integer? lst))
...
(define (filter-comp p lst)
  (cond [(empty? lst) '()]
        [(p (first lst)) (filter-comp p (rest lst))]
        [else (cons (first lst) (filter-comp p (rest lst)))])
)
...
(define (partition p lst)
  (list (filter p lst) (filter-comp p lst))
)
...
(define (minim lst)
  (fold-right min-of-2 (first lst) (rest lst)))

(define (menor-quociente x lst)
  ;(cond [(if (empty? lst)) x]
   ;     [(if (> (length (filter zero? lst)) 0)) x]
  ;(minim (map (lambda (y) (/ x y)) lst) )

  (if (empty? lst)
      x
      (minim (map (lambda (y) (/ x y)) (filter-comp zero? lst))))
)
...
(define-test-suite test-menor-quociente
  (test-equal? "Lista vazia"
               (menor-quociente  12 '())
               12)
  (test-equal? "42 com real"
               (menor-quociente 42 (list 2.1 3.15 4.2))
               10.0 )
  (test-equal? "12"
               (menor-quociente 12 (list 2 3 4))
               3)
  (test-equal? "12"
               (menor-quociente 12 (list -2 -3 -4))
               -6)
  (test-equal? "12"
               (menor-quociente 12 (list 0.2 0.3 0.4))
               30.0)
  (test-equal? "dividindo com 0"
               (menor-quociente 12 (list 2 3 0))
               4))
