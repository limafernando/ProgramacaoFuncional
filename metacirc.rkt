...
(define (eval-define args)
  (if (list? (first args))
      (eval-fun (first(first args)) (rest(first args)))
      (eval-define2 args)))

(define (eval-define2 args)
...
(define (eval-begin args)
  (foldl (lambda (e1 a) (eval e1)) (void) args))
...
(adiciona-primitiva 'string-length string-length)

(adiciona-primitiva 'string-append string-append)
...
(define prog-fat
  '(begin
     (define fat(lambda (n) (if (= n 0) 1 (* n (fat (sub1 n))))))

  (fat 5)))

;;Questão 5

;;Os números inteiros da nossa linguagem podem ter tamanho arbitrário porque os inteiros de racket são de precisão arbitrária,
;; então nossa linguagem herda essa característica de racket
...
(module+ test
  ...
  (define-test-suite test-eval
    ...
    (test-equal? "begin" (eval '(begin (+ 3 2) (* 3 2))) 6)
    (test-equal? "retorno de void" (eval '(begin)) (void))
    (test-equal? "definindo variável" (eval '(begin (define x 2) x)) 2)
    (test-equal? "variavel temporária" (eval-temp-env '() '(begin (define y 3) (* y 3))) 9)
    (test-equal? "testanto primitiva" (eval '(>= 2 2)) #t)
    (test-equal? "teste strings" (eval '(string-append "Hello," " World")) "Hello, World"))
  (run-tests test-eval))
