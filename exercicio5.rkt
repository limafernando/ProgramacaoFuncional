...
(define (adiciona-prefixo pre lista-assuntos)
  (if (equal? (length lista-assuntos) 1)
      (map (lambda (x) (~a "[" pre "]" x)) lista-assuntos)
      (map (lambda (x) (~a "[" pre "] " x)) lista-assuntos))
)
...
(define-test-suite test-and-or-lista
  (test-equal? "and com lista vazia" (and-lista '()) #t) ;Mudei o teste, tava retornando #f
  ...
  (test-equal? "or com lista vazia" (or-lista '()) #f) ;Mudei o teste, tava retornando #t
...
(define (pontuacao-final-fr ini lstded)
  (fold-right - (soma-lista lstded) (list ini))
)
...
(define (concat-lista lista-str)
  (fold-left ~a "" lista-str)
)

;;Podemos usar as duas funções e o resultado é o mesmo. A única diferença é que a right concatena a partir do elemento final
;;com os elementos anteriores e a left concante o a partir do elemento inicial
...
;;
;;(define (concat-lista-sep lista-str sep acc)
  ;;(fold-right ~a sep lista-str)
  ;;(fold-right (lambda (x) (~a x sep)) sep lista-str)
  ;(fold-right ~a (fold-right ~a sep (first lista-str)) )

  ;;(if (empty? lista-str)
    ;;  ""
      ;;(if (and (equal? (length lista-str) 1) (equal? acc ""))
        ;;  (first lista-str)

          ;;(if (equal? (length lista-str) 1)
            ;;  (~a acc sep (first lista-str))

              ;;(if (equal? acc "")
                ;;  (concat-lista-sep (rest lista-str) sep (~a acc (first lista-str)))

                  ;;(concat-lista-sep (rest lista-str) sep (~a acc sep (first lista-str))))
;;))))

(define (concat-lista-sep lista-str sep)
  (define (aux-concat lst s acc)

  (if (empty? lst)
      acc

      (if (equal? (length lst) 1)
          (aux-concat (rest lst) s (cons (~a (first lst) "") acc))
          (aux-concat (rest lst) s (cons (~a (first lst) s) acc)))))

  (fold-right ~a "" (reverse (aux-concat lista-str sep '()))))
