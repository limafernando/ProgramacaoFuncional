...
(define-test-suite test-questao3
  (test-equal? "lista heterogenea 1"           (remove-primeiro  'a '(1 3 'a 5 7))       '(1 3 5 7))
  (test-equal? "lista heterogenea 2"        (remove-todos 'a '(1 3 'a 5 7))       '(1 3 5 7))
  (test-equal? "lista caractere 1"           (remove-primeiro  'a '('a 'b 'c 'a))       '('b 'c 'a))
  (test-equal? "lista caractere 2"        (remove-todos 'a '('a 'b 'c 'a))       '('b 'c)))

;;funcionam tanto para outros tipos, quanto para listas heterogêneas 
...
(define (remove-duplicatas lst)
  (if (empty? lst)
      '()
      (if (pertence? (first lst) (rest lst))
...
(define (uniao c1 c2)
  (if (empty? c1)
      ...
      (if (empty? c2)
          c1
          (cons (first c1) (uniao (rest c1) (remove-todos (first c1) c2)))))
)
