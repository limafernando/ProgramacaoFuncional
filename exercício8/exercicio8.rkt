...
(define (mult-vetor-fold vec)
  (for/fold ([acc 1])     ;; acumulador é uma variável chamada acc, valor inicial 1
            ([x vec])      ;; x é a variável do loop, em cada iteração x será um elemento de vec
    (* acc x)))           ;; expressão que atualiza o valor do acumulador para a próxima iteração

;; b) o menor valor entre os números de um vetor
(define (min-vetor-fold vec)
  (for/fold ([acc +inf.0])     ;; acumulador é uma variável chamada acc, valor inicial +inf.0
            ([x vec])      ;; x é a variável do loop, em cada iteração x será um elemento de vec
    (min acc x)))           ;; expressão que atualiza o valor do acumulador para a próxima iteração

;; c) a concatenação de todas as strings de um vetor de strings
(define (concat-vetor-fold vec)
  (for/fold ([acc ""])     ;; acumulador é uma variável chamada acc, valor inicial ""
            ([x vec])      ;; x é a variável do loop, em cada iteração x será um elemento de vec
    (string-append acc x)))           ;; expressão que atualiza o valor do acumulador para a próxima iteração

;; d) a maior string em um vetor de strings
(define (max-str-vetor-fold vec)
  (for/fold ([acc ""])     ;; acumulador é uma variável chamada acc, valor inicial ""
            ([x vec])      ;; x é a variável do loop, em cada iteração x será um elemento de vec
    (if (> (string-length acc) (string-length x))
     acc
     x)))           ;; expressão que atualiza o valor do acumulador para a próxima iteração

;; Escreva testes para essas funções.
(define-test-suite test-vetor-fold
  (test-equal? "multiplicação" (mult-vetor-fold (vector 1 2 3 4)) 24)
  (test-equal? "menor valor" (min-vetor-fold (vector 1 2 3 4)) 1.0)
  (test-equal? "concatenação" (concat-vetor-fold (vector "teste" "teste1")) "testeteste1")
  (test-equal? "maior string" (max-str-vetor-fold (vector "teste" "teste1" "teste100" "teste12")) "teste100"))
...
(define (read-sum-lines)
  (define arq (open-input-file "input1.txt"))
  (define ret (sum-lines arq 0))
  (close-input-port arq)

  ret
)

(define-test-suite test-io-arq
  (test-equal? "io arq" (read-sum-lines) 510))
