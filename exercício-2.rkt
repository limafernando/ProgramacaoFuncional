...
(define lista-ex5 '(1 2 3 4 5)) ;;(define lista-ex5 list(1 2 3 4 5))
...
(define (max-lista l)
  (if (empty? l) 0
      (maior (first l) (max-lista (rest l)))))
...
(define (elemento-n lista n)
  (if (empty? lista) #f
    (if (>= n (length lista)) #f
        (if (zero? n) (first lista) (elemento-n (rest lista) (sub1 n))))))
...
(define (quadrado-lista l)
  (if (empty? l) '()
      (cons (expt (first l) 2) (quadrado-lista (rest l)))))
